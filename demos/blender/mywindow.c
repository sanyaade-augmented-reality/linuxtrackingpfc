/**
 * $Id: mywindow.c 16461 2008-09-10 13:02:58Z blendix $
 *
...
// para simplificar codigo, se aplica inlining
	...
	if(win) {
		...
		// FOV
		/*float fov = atan(y1/n)+atan(y2/n);
		printf("El fov original era: %f\n", 360*fov/6,28);
		fov = atan(frup/n)+atan(frdown/n);
		printf("El nuevo es: %f\n", 360*fov/6,28);*/

		// En la vista original, el programa asumia cierto fov
		// para tener ese fov hay que estar centrado mirando a la ventana y estar
		// a una cierta distancia, esa distancia es originalz
		originalz = (scry*n)/(y2-y1);
		// como obsz esta dado desde el plano del display, lo que hay que mover
		// en realidad la camara no es obsz, sino obsz-originalz
		// Al hacer esto, si no modificamos znear y zfar, podemos estar
		// eliminando objetos que antes eran visibles y que ahora quedan demasiado
		// cerca o demasiado lejos.
		// idealmente, deberiamos modificar znear y zfar con la distancia que se mueve
		// la camara. Eso no es problema si estamos alejando la camara (aumentamos zfar)
		// el problema es si la acercamos, ya que valores muy pequeños de znear
