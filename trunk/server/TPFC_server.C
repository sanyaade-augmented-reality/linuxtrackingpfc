	...
	if ( input[0].compare("device")==0 || input[0].compare("dev")==0|| input[0].compare("d")==0){
	  ...
	  if ( input[1].compare("artoolkit")==0 || input[1].compare("art")==0){
	    if (artcreated){
	      ...
	      printf("Los devices artoolkit requieren como parametro el tamaño del marcador a usar y el umbral de deteccion.\n");
	    }else{
	      // añadimos el dispositivo
	      dev.push_back( new TPFC_device_artoolkit(dev.size(), str2int(input[2]),str2int(input[3]),argc, argv) );
...
;
	...
	if (s.compare("help")==0 || s.compare("?")==0 || s.compare("h")==0){
	  ...
	  printf("dev artoolkit (art) <tamaño del marcador en milimetros> <umbral de deteccion>\n");
